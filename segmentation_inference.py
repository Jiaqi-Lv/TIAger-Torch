...
def process_image_tile_to_segmentation(
    ...
    if isinstance(segModels, list):
        tum_thresh = 0.15  # 0.20 #0.25 #0.3 #80
        stroma_thresh = 0.35  # 0.35 #0.30 #0.20 #0.25 #70
    ...
    pred_tum_mask = cv2.morphologyEx(
        np.uint8(pred_tum_mask), cv2.MORPH_OPEN, np.ones((open_disk_r, open_disk_r))
    )
    pred_stroma_mask = pred_stroma_ens > stroma_thresh
    prediction = np.zeros_like(
        pred_stroma_ens
    )  # Aggregating both tumour and stroma in one map, you can separate later if you want
    prediction[pred_stroma_mask > 0] = 2  # 127
    prediction[pred_tum_mask > 0] = 1  # 255
    # resize output from 10X back to 20X
    # seg_map = cv2.resize(prediction, (orig_dims[1], orig_dims[0]), interpolation=cv2.INTER_NEAREST)
    seg_map = prediction
    seg_map = seg_map * tissue_mask_tile
    seg_map = cropping_center(
        seg_map, (orig_dims[0] // 2, orig_dims[1] // 2), batch=False
    )
